...
const ClassicTemplate: React.FC<CertificateTemplateProps> = ({ data, customConfig, isEditable = false, onLabelChange }) => {
    ...
    const DraggableBox = ({ id, children, defPos, ...rest }: { id: string, children: React.ReactNode, defPos: { x: number, y: number }, [key: string]: any }) => {
        ...
        return (
            ...
            >
                <div className={`relative border-2 border-transparent hover:border-indigo-400 group-hover/draggable:border-indigo-400 rounded p-1 ${draggingId === id ? 'border-indigo-600 shadow-xl bg-white/10' : ''}`}
    ...
    const textColor = customConfig?.textColor || '#1a1a1a';
    ...
    return (
        ...
        >
                ...
                <>
                    <div className="absolute inset-0 p-8 z-0">
                        <div className="w-full h-full border-[6px] border-solid border-[#8b4513] relative p-1 box-border">
                            <div className="w-full h-full border-[2px] border-solid border-[#8b4513] box-border"></div>
                        </div>
                    </div>
                    {['absolute top-[2.5rem] left-[2.5rem] w-16 h-16',
                      'absolute top-[2.5rem] right-[2.5rem] w-16 h-16 rotate-90',
                      'absolute bottom-[2.5rem] right-[2.5rem] w-16 h-16 rotate-180',
                      'absolute bottom-[2.5rem] left-[2.5rem] w-16 h-16 -rotate-90'
                    ].map((cls, i) => (
            ...
            {isVisible('logo') && (!customConfig?.logos || customConfig.logos.length === 0) && (
                <DraggableBox id="logo" defPos={{ x: 50, y: 15 }}>
                    <div className="relative group/logo-container">
                        <img src={customConfig?.logoImage || "/educheck_logo.png"}
                            className="h-24 w-auto object-contain drop-shadow-sm pointer-events-none" alt="Logo" />
            ...
            {customConfig?.logos?.map((logo, index) => (
                <DraggableBox key={`logo-${index}`} id={`logo-${index}`} defPos={{ x: 30 + index * 20, y: 15 }}>
            ...
            {/* Title */}
            {isVisible('title') && (
                <DraggableBox id="title" defPos={{ x: 50, y: 30 }}>
                    <h1 className="text-6xl font-bold uppercase whitespace-nowrap leading-none text-center" style={{ color: getStyle('title').color || '#8b4513', fontFamily: "'Dancing Script', cursive" }}>
            ...
            {isVisible('recipient') && (
                <>
                    <DraggableBox id="presentedTo" defPos={{ x: 50, y: 40 }}>
                        <p className="text-xl italic whitespace-nowrap" style={{ color: getStyle('presentedTo').color || textColor }}>
                            <Editable val={labels.presentedTo} k="presentedTo" />
                        </p>
                    </DraggableBox>
                    <DraggableBox id="recipient" defPos={{ x: 50, y: 48 }}>
                        <h2 className="text-4xl font-bold whitespace-nowrap" style={{ color: getStyle('recipient').color || '#2c1810' }}>
                            {data.recipientName}
                        </h2>
                    </DraggableBox>
                </>
            ...
            {/* Event */}
            <DraggableBox id="eventStr" defPos={{ x: 50, y: 56 }}>
                <p className="text-lg text-center whitespace-nowrap" style={{ color: getStyle('eventStr').color || textColor }}>
                    <Editable val={labels.eventPrefix} k="eventPrefix" />
                </p>
            </DraggableBox>
            {isVisible('eventName') && (
                <DraggableBox id="eventName" defPos={{ x: 50, y: 63 }}>
                    <h3 className="text-2xl font-bold uppercase whitespace-nowrap" style={{ color: getStyle('eventName').color || '#8b4513' }}>{data.eventName}</h3>
                </DraggableBox>
            )}

            {/* Entry No — NO font-serif, inherits parent font for proper Vietnamese rendering */}
            {isVisible('entryNo') && (
                <DraggableBox id="entryNo" defPos={{ x: 20, y: 80 }}>
                    <p className="text-sm font-medium opacity-80 whitespace-nowrap" style={{ color: getStyle('entryNo').color || textColor }}>
                        <Editable val={labels.entryNo} k="entryNo" />
                    </p>
                </DraggableBox>
            )}

            {/* Date — NO font-serif, inherits parent font for proper Vietnamese rendering */}
            {isVisible('date') && (
                <DraggableBox id="date" defPos={{ x: 20, y: 85 }}>
                    <p className="text-sm font-medium opacity-80 whitespace-nowrap" style={{ color: getStyle('date').color || textColor }}>
            ...
            {isVisible('qr') && (
                <DraggableBox id="qr" defPos={{ x: 50, y: 85 }}>
                    <div className="flex flex-col items-center pointer-events-none">
                        <img src={data.verifyQR} alt="QR" className="w-20 h-20 border-2 border-[#8b4513]" />
                        <p className="text-[10px] tracking-widest mt-1">{data.verifyCode}</p>
                    </div>
                </DraggableBox>
            )}
            ...
            {isVisible('signature') && (
                <DraggableBox id="signature" defPos={{ x: 80, y: 85 }}>
                    <p className="italic font-bold uppercase whitespace-nowrap" style={{ color: getStyle('signature').color || textColor }}>
