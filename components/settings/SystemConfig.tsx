...
const SystemConfig: React.FC<SystemConfigProps> = ({ onBack }) => {
    ...
    const getConfigIcon = (key: string): React.ReactNode => {
        if (key.includes('checkin')) return ConfigIcons.checkin;
        if (key.includes('boarding')) return ConfigIcons.boarding;
        if (key.includes('manual')) return ConfigIcons.manual;
    ...
    const pointConfigs = configs.filter(c => c.key.includes('points'));
        ...
        <div className="space-y-6">
                    ...
                    {pointConfigs.map(config => (
                        <div key={config.key} className="bg-slate-50 rounded-2xl p-4">
                            <div className="flex items-center gap-2 mb-2 text-slate-600">
                                ...
                                <p className="text-sm font-bold">{config.description || config.key}</p>
                            </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                ...
                <p className="text-slate-500 mb-4">
                    Nhấn nút dưới để tự động tạo các sheet mới cần thiết trong Google Sheet (ví dụ: Event_Participants, v.v.)
                </p>
                <button
                    onClick={handleInitSystem}
                    disabled={isInitializing}
                    className={`px-6 py-3 rounded-xl font-bold flex items-center gap-2 ${isInitializing
